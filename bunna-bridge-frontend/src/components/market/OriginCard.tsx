...
export default function OriginCard({ origin, featured = false }: { origin: OriginSummary; featured?: boolean; key?: string | number }) {
  return (
    <Link
    ...
    >
      ...
      <svg viewBox="0 0 240 240" aria-hidden style={{ position: "absolute", right: featured ? "-20px" : "-40px", bottom: featured ? "-30px" : "-46px", width: featured ? "220px" : "170px", opacity: 0.35 }}>
        {[46, 70, 94, 118].map((r) => (
          <circle key={r} cx="120" cy="230" r={r} fill="none" stroke="rgba(255,255,255,0.5)" strokeWidth="1" />
        ))}
      </svg>
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(8,16,12,0.62), transparent 62%)" }} />
